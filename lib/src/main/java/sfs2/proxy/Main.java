 ...
 * - My parser can probably get exploited for a denial of service through nested SFSObjects
 * - Proxy only works for 1 client at a time but I think this is ok. The newest client connection
 *    *should* bump any older connection.
 * - allow for staying at most recent request
 * - Converting byte array to sfsObject then back to byte array creates different sequence of bytes
 *   NullPointerExceptions get thrown in the response handling since (i think) the code doesn't properly
 *   handle commands. I think this is ok since it doesn't really need to, just needs to parse data and pass it on.
 */
...
public class Main  {
	...
	public void updateTextBox(List list, SFSDataManipulation sfsDM, Text textBox) {
		...
		try {
			sfsObj = SFSDataManipulation.decode(SFSDataManipulation.hexStringToByteArray(item), handler);
	...
	private void handleReplayClick(SelectionEvent event, Text textBox, List list, DataOutputStream outputStream, Socket socket) {
	       ...
	       item = list.getItem(i);
	       b = SFSDataManipulation.hexStringToByteArray(item);
